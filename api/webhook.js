...
// Parsea el texto del datepicker de Smile
// Ejemplo: "[Smile Datepicker] Información de la reserva para la órden #31075: 27/3/2026, entre las 15:00 y las 22:00 (America/Argentina/Buenos_Aires)."
...
// Detecta si el pago es en efectivo
function esEfectivo(order) {
  const metodos = order.payment_details || [];
  if (Array.isArray(metodos)) {
    return metodos.some(p =>
      (p.payment_method || "").toLowerCase().includes("efectivo") ||
      (p.method || "").toLowerCase().includes("efectivo") ||
      (p.type || "").toLowerCase().includes("cash")
    );
  }
  const mp = (order.payment_status || "").toLowerCase();
  return mp.includes("efectivo") || mp.includes("cash");
}
...
function ordenAEnvio(order) {
  ...
  const efectivo    = esEfectivo(order) ||
    formaPago.toLowerCase().includes("efectivo") ||
    formaPago.toLowerCase().includes("cash");
  ...
  return {
    ...
    // Notas
    notasOrden,         // editable
    notasCliente,       // solo lectura
    datepickerRaw,      // referencia al texto original del datepicker
    // Fechas y turno (del datepicker)
    fechaVenta:      (order.created_at || "").split("T")[0],
    fecha,              // del datepicker
    turno,              // AM o PM del datepicker
    // Asignacion (manual)
    trans:           "",
    estado:          "sin_asignar",
    importe:         0,
...
export default async function handler(req, res) {
  ...
  const db = initFirebaseAndGetDb();
  const docRef = db.collection("envios").doc(String(order.id));
  const existing = await docRef.get();

  if (existing.exists) {
    return res.status(200).json({ ok: true, skipped: "already exists" });
  }

  const envio = ordenAEnvio(order);
  await docRef.set(envio);

  return res.status(200).json({ ok: true, saved: envio.id, fecha: envio.fecha, turno: envio.turno });
}
