...
import { ordenAEnvio, ordenAOtroPedido, parsearDatepicker, getPagoEstadoInicial, clasificarOtro, esEfectivo } from "./_tn.js";
...
export default async function handler(req, res) {
  ...
  if (esUMP) {
    ...
    if (topicFinal === "order/updated") {
      ...
      // El datepicker de Smile escribe en la nota del cliente (order.note), no en owner_note
      ...
      const nuevoEstadoPago = getPagoEstadoInicial(order);
      if (data.pagoEstado !== "cuenta_corriente") {
        update.pagoEstado = nuevoEstadoPago;
      }

      const efectivo = esEfectivo(order);

      if (efectivo) {
        // Efectivo: el importe se cobra al entregar, NO por TN.
        // Nunca borrar cobranza por un webhook de actualización genérica.

        // Restaurar cobranza si fue borrada por el bug anterior
        if (!data.cobranza && !data.cobranzaRecibida) {
          update.cobranza = parseFloat(order.total) || 0;
          console.log("WEBHOOK COBRANZA_RESTORED", order.id, update.cobranza);
        }

        // Marcar como cobrado solo cuando TN confirma payment_status=paid
        // (alguien lo marcó como cobrado en el panel de TN)
        if (order.payment_status === "paid" && !data.cobranzaRecibida) {
          update.cobranzaRecibida = true;
          update.cobranzaFecha = new Date().toISOString().split("T")[0];
          // Asegurarse de que el importe quede registrado aunque haya sido borrado
          if (!data.cobranza && !update.cobranza) update.cobranza = parseFloat(order.total) || 0;
          console.log("WEBHOOK EFECTIVO_COBRADO", order.id, data.cobranza || order.total);
        }
      } else {
        // Pago online (Pago Nube, MP, etc.): limpiar cobranza cuando TN confirma el pago
        if ((order.payment_status === "paid" || order.payment_status === "authorized") && !data.cobranzaRecibida) {
          update.cobranza = null;
          console.log("WEBHOOK COBRANZA_CLEARED", order.id, "pago online confirmado");
        }
      }
